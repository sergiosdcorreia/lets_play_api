...
async function createTeam(req, res) {
    try {
        ...
        // Validate input
        const validatedData = validation_1.teamSchema.parse(req.body);
        // Check if sub-manager exists
        if (validatedData.subManagerId) {
            const subManager = await prisma_1.prisma.user.findUnique({
...
async function updateTeam(req, res) {
    try {
        ...
        // Validate input
        const validatedData = validation_1.teamUpdateSchema.parse(req.body);
        ...
        // Update team
        const updated = await prisma_1.prisma.team.update({
            where: { id },
            data: validatedData,
            include: {
                manager: {
...
async function invitePlayer(req, res) {
    try {
        ...
        const isMemberCoordinator = team.members.some((m) => m.userId === userId && m.role === "coordinator");
        ...
        const existingInvite = await prisma_1.prisma.teamInvite.findFirst({
            where: {
                teamId,
                email,
                status: "pending",
...
async function rsvpToTeam(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ error: "Not authenticated" });
        }
        ...
        // Validate input
        const validatedData = validation_1.teamMemberRsvpSchema.parse(req.body);
        ...
        res.json({
            message: `Successfully ${validatedData.status === "active" ? "accepted" : "declined"} team invitation`,
            member: updated,
        });
    }
    catch (error) {
...
async function leaveTeam(req, res) {
    try {
        ...
        if (team.managerId === req.user.id) {
            return res.status(400).json({
                error: "Team manager cannot leave. Transfer ownership or delete the team.",
            });
        }
        // Find user's membership
        const membership = await prisma_1.prisma.teamMember.findFirst({

...
import { matchEventSchema } from "../utils/validation";
...
export async function recordMatchEvent(req: Request, res: Response) {
  try {
    ...
    // Validate input
    const validatedData = matchEventSchema.parse(req.body);
    ...
    const playerInMatch = match.players.some(
      (p: { userId: string }) => p.userId === validatedData.playerId
    );
...
export async function getMatchEvents(req: Request, res: Response) {
  try {
    ...
    const summary = {
      ...
      assists: events.filter((e: any) => e.eventType === "assist").length,
      yellowCards: events.filter((e: any) => e.eventType === "yellow_card")
        .length,
  ...
  try {
    ...
    const stats = {
      ...
      goalsPerMatch:
        matchesPlayed > 0
          ? (
              events.filter((e: any) => e.eventType === "goal").length /
              matchesPlayed
            ).toFixed(2)
          : "0.00",
      assistsPerMatch:
        matchesPlayed > 0
          ? (
              events.filter((e: any) => e.eventType === "assist").length /
              matchesPlayed
            ).toFixed(2)
          : "0.00",
    };
    ...
    const recentEvents = events
      .sort((a: any, b: any) => b.match.date.getTime() - a.match.date.getTime())
      .slice(0, 10);
...
export async function getMatchStatistics(req: Request, res: Response) {
  try {
    const { id } = req.params; // match ID
    ...
    const teamAGoals = match.events.filter(
      (e: any) =>
        e.eventType === "goal" &&
        teamAPlayers.some((p: any) => p.userId === e.playerId)
    ).length;

    const teamBGoals = match.events.filter(
      (e: any) =>
        e.eventType === "goal" &&
        teamBPlayers.some((p: any) => p.userId === e.playerId)
    ).length;

    // Top scorer(s) in this match
    const playerGoals: Record<string, any> = {};
    match.events
      .filter((e: any) => e.eventType === "goal")
      .forEach((e: any) => {
    ...
    // Event summary
    const summary = {
      totalGoals: match.events.filter((e: any) => e.eventType === "goal")
        .length,
      totalAssists: match.events.filter((e: any) => e.eventType === "assist")
        .length,
      yellowCards: match.events.filter(
        (e: any) => e.eventType === "yellow_card"
      ).length,
      redCards: match.events.filter((e: any) => e.eventType === "red_card")
        .length,
    };

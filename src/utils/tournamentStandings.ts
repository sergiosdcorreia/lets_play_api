...
export async function updateTournamentStandings(result: MatchResult) {
  ...
  const homeTeamStats = await prisma.tournamentTeam.findFirst({
    where: {
      tournamentId,
      teamId: homeTeamId,
    },
  ...
  const awayTeamStats = await prisma.tournamentTeam.findFirst({
    where: {
      tournamentId,
      teamId: awayTeamId,
    },
  ...
  console.log(
    `   Home (${homeTeamId}): ${homeScore} - Away (${awayTeamId}): ${awayScore}`
  );
